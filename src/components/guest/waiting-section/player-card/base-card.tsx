...
function Image({ player }: { player: User }) {
    ...
    return (
        <>
            {loading && <div className='w-20 h-20 bg-gray-400 rounded-full animate-pulse'></div>}
            <img src={player.avatar} alt={player.name} className={cn('rounded-full invisible opacity-0 transition-opacity duration-[250ms] w-20 h-20 absolute', {'relative opacity-100 visible': !loading})} onLoad={() => setLoading(false)} />
        </>
    )
}
